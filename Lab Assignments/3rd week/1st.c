...
int longest_palindromic_substring_length(char *s, int n) {
    ...
    // Initialize a 2D table to store lengths of palindromic substrings
    int dp[n][n];

    // Initialize all values in dp array to 0
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
            dp[i][j] = 0;
        }
    ...
    // Marking all substrings of length 1 as palindromic
    for (int i = 0; i < n; i++) {
        dp[i][i] = 1;
    }
